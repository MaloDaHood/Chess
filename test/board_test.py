...
class Board:
    ...
    # Displays a square on the origin of the player's drag
    def display_marker(self, window :pygame.surface.Surface, position :"tuple[int, int]", is_dragging :bool) -> None:
        # We check if the player is dragging a letter
        if is_dragging:
            pygame.draw.rect(window, (96,193,216), pygame.Rect((position[1] * 100) + 10, (position[0] * 100) + 10, 80, 80))
    
    # # Displays a square on the origin of the player's drag
    # def display_markers(self, window :pygame.surface.Surface, legal_moves, position :"tuple[int, int]", is_dragging :bool) -> None:
    #     # We check if the player is dragging a letter
    #     if is_dragging:
    #         pygame.draw.rect(window, (96,193,216), pygame.Rect((position[1] * 100) + 10, (position[0] * 100) + 10, 80, 80))
